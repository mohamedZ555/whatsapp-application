    ...
    <div>
      ...
      {!unlimited && !notAllowed && limit > 0 && pct >= 90 && (
        <p className="mt-0.5 text-[11px] text-rose-500 font-medium">
          {pct >= 100 ? limitReachedLabel : `${limit - used} remaining`}
        </p>
      )}
    </div>
  );
...
export default async function SubscriptionPage() {
  ...
  // Translated status labels for history table
    ...
    <div className="space-y-6">
          ...
          <div className="text-right">
            ...
            {billingCycle === "yearly" && currentPlan.pricing.monthly > 0 && (
              <p className="text-xs text-emerald-300 mt-0.5">
                vs ${currentPlan.pricing.monthly * 12}
                {t("perYear")} monthly
              </p>
            )}
          </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {items.map((item) => (
            <UsageBar
              key={item.key}
              label={t(`usageLabel.${item.key}` as any, { default: item.key })}
              used={item.used}
              limit={item.limit}
              notIncludedLabel={t("notIncluded")}
              limitReachedLabel={t("limitReachedUpgrade")}
            />
          ))}
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-3 border-t border-gray-100 pt-4">
          <FeaturePill
            label={t("aiChatBotLabel")}
            enabled={currentPlan.features.aiChatBot}
          />
          <FeaturePill
            label={t("apiAccessLabel")}
            enabled={currentPlan.features.apiAccess}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <SubscriptionPlanCards
          ...
          endsAt={
            activeSub?.endsAt ? new Date(activeSub.endsAt).toISOString() : null
          }
          plans={plans}
        />
      </div>
