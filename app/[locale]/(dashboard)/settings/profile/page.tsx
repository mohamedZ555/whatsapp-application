...
export default function SettingsProfilePage() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      .then((r) => r.json())
      .then((data) => setForm({ profile_name: data.profile_name ?? '', profile_email: data.profile_email ?? '' }))
      .catch(() => {});
  }, []);
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            onChange={(e) => setForm((s) => ({ ...s, profile_name: e.target.value }))}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setForm((s) => ({ ...s, profile_email: e.target.value }))}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
          />
        </div>
