  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 overflow-y-auto py-8">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg mx-4 my-auto">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Edit Plan — {plan.title}</h2>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Trial Days</label>
              <input
                type="number"
                min={0}
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="plan-enabled" className="text-sm text-gray-700">Enabled (visible to vendors)</label>
          </div>

          {/* Pricing */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-2">Pricing (USD)</label>
            <div className="grid grid-cols-2 gap-3">
              <div>
          ...
          {/* Feature Limits */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-2">Feature Limits (-1 = Unlimited, 0 = Disabled)</label>
            <div className="grid grid-cols-2 gap-3">
              {(
                [
                  { key: 'contacts', label: 'Contacts' },
                  { key: 'campaignsPerMonth', label: 'Campaigns / Month' },
                  { key: 'botReplies', label: 'Bot Replies' },
                  { key: 'botFlows', label: 'Bot Flows' },
                  { key: 'contactCustomFields', label: 'Custom Fields' },
          ...
          {/* Stripe Price IDs */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-2">Stripe Price IDs (optional)</label>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1">Monthly Price ID</label>
                <input
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1">Yearly Price ID</label>
                <input
...
export default function AdminPlansPage() {
  ...
  async function savePlans(updated: PlansMap) {
    ...
    try {
      ...
      if (data.success) {
        setPlans(updated);
        setFeedback({ type: 'success', msg: 'Plans saved successfully.' });
      } else {
  ...
  async function handleReset() {
    if (!confirm('Reset all plans to defaults? This will discard any customizations.')) return;
    setSaving(true);
    try {
      const res = await fetch('/api/admin/plans', {
        method: 'PUT',
      ...
      const data = await res.json();
      if (data.success) { await load(); setFeedback({ type: 'success', msg: 'Plans reset to defaults.' }); }
    } catch { /* ignore */ }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        <div>
          <h1 className="text-[40px] font-normal leading-none tracking-tight text-emerald-950">Plans</h1>
          <p className="mt-1 text-sm text-slate-500">Manage subscription plans, pricing, and feature limits.</p>
        </div>
        <button
          onClick={handleReset}
          disabled={saving}
      ...
      {loading ? (
        <div className="rounded-md border border-emerald-100 bg-white p-10 text-center text-sm text-slate-400">Loading plans...</div>
      ) : (
              ...
              <div className="border-t border-gray-100 pt-3 space-y-1 text-sm">
                ...
                {plan.trialDays > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-800">{plan.trialDays} days</span>
                  </div>
                )}
              </div>

              <div className="border-t border-gray-100 pt-3 space-y-1 text-xs text-gray-500">
                <div className="flex justify-between"><span>Contacts</span><span className="font-medium text-gray-700">{featureDisplay(plan.features.contacts)}</span></div>
                <div className="flex justify-between"><span>Campaigns/mo</span><span className="font-medium text-gray-700">{featureDisplay(plan.features.campaignsPerMonth)}</span></div>
                <div className="flex justify-between"><span>Bot Replies</span><span className="font-medium text-gray-700">{featureDisplay(plan.features.botReplies)}</span></div>
                <div className="flex justify-between"><span>Bot Flows</span><span className="font-medium text-gray-700">{featureDisplay(plan.features.botFlows)}</span></div>
                <div className="flex justify-between"><span>Team Members</span><span className="font-medium text-gray-700">{plan.features.teamMembers === 0 ? 'None' : featureDisplay(plan.features.teamMembers)}</span></div>
                <div className="flex justify-between"><span>AI Chatbot</span><span className={plan.features.aiChatBot ? 'text-emerald-600 font-medium' : 'text-gray-400'}>{ plan.features.aiChatBot ? 'Yes' : 'No'}</span></div>
                <div className="flex justify-between"><span>API Access</span><span className={plan.features.apiAccess ? 'text-emerald-600 font-medium' : 'text-gray-400'}>{plan.features.apiAccess ? 'Yes' : 'No'}</span></div>
              </div>
