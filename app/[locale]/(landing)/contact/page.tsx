...
export default function ContactPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      <div className="mb-8">
        <Link href="/" className="inline-flex items-center gap-1 text-sm text-green-600 hover:underline mb-6">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
        {status === 'success' ? (
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {status === 'error' && (
              <p className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{t('errorDesc')}</p>
            )}
      ...
      <div className="mt-8 text-center text-sm text-gray-400">
        <Link href="/register" className="text-green-600 hover:underline font-medium">{tHero('getStarted')}</Link>
        {' · '}
        <Link href="/page/terms" className="hover:text-gray-600">{tFooter('terms')}</Link>
        {' · '}
        <Link href="/page/privacy" className="hover:text-gray-600">{tFooter('privacy')}</Link>
      </div>
    </div>
  );
}
